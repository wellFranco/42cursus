...
void	init_smaller_smaller_than(char **args)
{
	...
	close(fd);
	fd = open(".heredoc", O_RDONLY); // PQ RAIOS EU TENHO Q FECHAR ABRIR PRA CONSEGUIR LER O Q EU MESMO ESCREVI??
	// if ()
	// {
	// 	???
	// }
...
void	finish_smaller_smaller_than(int new_stdin)
{
	...
	// printf("new_stdin: %d\n", new_stdin);
	close(new_stdin); // ESSE FD E FECHADO APOS O LOOP DE ARGUMENTOS // SERA Q TEM PROBLEMA FECHAR O MESMO FD DUAS VZS?
	unlink(".heredoc");
}
